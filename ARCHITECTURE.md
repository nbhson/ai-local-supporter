...
2. **Celery Workers ([tasks.py](tasks.py)):**
   Là các luồng xử lý chạy hoàn toàn độc lập với Flask. Thực hiện các tác vụ nặng: Trích xuất nội dung văn bản thông qua Extractor Service, chạy OCR, tạo chunks văn bản, sinh vector nhúng và nạp vào ChromaDB.
...
> **Tại sao cần Redis ở đây?**
> - Flask xử lý HTTP request chỉ nên mất `<100ms`.
> - Việc trích PDF (có thể 5-30s) không thể làm trong HTTP request.
> - Redis là "bưu điện" — nhận task, lưu lại, chờ Celery đến lấy.

#### Bước 2️⃣ — Celery Worker kéo task từ Redis
...
> **Tại sao lại cắt nhỏ (chunking)?**
>
> ❌ **Nếu không cắt:**
> - Gửi toàn bộ 5000 ký vào LLM model.
> - LLM phải đọc toàn bộ → chậm, tốn tài nguyên.
> - Nếu file 100 trang → không fit vào context window.
>
> ✅ **Nếu cắt thành chunks:**
> - Mỗi chunk đại diện cho một ý chính riêng.
> - Khi user hỏi → chỉ cần 4 chunks liên quan nhất.
> - LLM xử lý nhanh hơn 10x.

#### Bước 3️⃣ — Tính toán Vector Embeddings (Phần quan trọng nhất)

> **Vector embedding là gì?**

```
Chunk text: "Hạn thanh toán: ngày 31/12/2024"
        ↓
Thư viện fastembed (chạy trên CPU)
Model: BAAI/bge-small-en-v1.5
        ↓
Vector: [0.2, -0.5, 0.8, 0.1, ..., 0.3]  ← 384 chiều
```

Đây là "DNA" của đoạn text — giúp tìm những đoạn tương tự.

> **Tại sao phải tính vector?**

```
Khi user hỏi: "Hạn thanh toán là khi nào?"
        ↓
Tính vector câu hỏi: [0.21, -0.48, 0.82, 0.11, ..., 0.32]
        ↓
ChromaDB so sánh vector này với tất cả vector của chunks
        ↓
Tìm 4 chunks có vector "gần nhất" (cosine similarity)
        ↓
Lấy chunks tương tự: [Chunk #5, Chunk #8, Chunk #12, Chunk #3]
        ↓
Gộp lại làm CONTEXT
        ↓
Gửi vào Ollama: "Dựa vào context này, trả lời: hạn thanh toán là khi nào?"
```

> **Công nghệ đáp ứng điều này:**
> - **fastembed:** Tính vector siêu nhanh trên CPU (không cần GPU).
> - **ChromaDB:** Database lưu vector + tìm kiếm vector similarity (nearest neighbors).

#### Bước 4️⃣ — Lưu vào ChromaDB (Vector Database)

```
Celery → Lưu tất cả chunks + vectors vào ChromaDB
        ↓
Collection: sess_abc123xyz
[
  {id: "chunk_1", text: "Hạn thanh toán: ngày 31/12...", vector: [...]},
  {id: "chunk_2", text: "Bên A thanh toán cho Bên B...", vector: [...]},
  {id: "chunk_3", text: "Phương thức thanh toán: chuyển khoản...", vector: [...]},
  ...
]
        ↓
✅ Cập nhật DB: status = "ready"
```

#### Bước 5️⃣ — Frontend polling (UI chờ tới khi ready)

```
👀 Frontend mỗi 2 giây:
   GET /api/doc/status/abc123xyz

Loop:
  ├─ Lần 1: {"status": "processing"}
  ├─ Lần 2: {"status": "processing"}
  ├─ Lần 3: {"status": "processing"}
  └─ Lần 4: {"status": "ready"} ✅ → Mở khung chat
```

#### Bước 6️⃣ — User hỏi câu hỏi (Phần quan trọng nhất #2)

> **Luồng RAG trong Flask**
...
🔌 Flask /api/doc/chat
        ↓
1️⃣  Tính vector của câu hỏi (fastembed trên CPU):
    "Hạn thanh toán của hợp đồng là ngày bao nhiêu?"
    ↓
    Vector: [0.3, -0.2, 0.9, ..., 0.1]  (384 chiều)
        ↓
2️⃣  ChromaDB tìm kiếm:
    SELECT TOP 4 chunks
    WHERE cosine_similarity(question_vector, chunk_vector) > threshold
        ↓
    Kết quả: [chunk_5, chunk_8, chunk_1, chunk_3]

    chunk_5: "Hạn thanh toán: ngày 31/12/2024, được gia hạn tối đa 30 ngày"
    chunk_8: "Phương thức thanh toán: chuyển khoản ngân hàng hoặc tiền mặt"
    chunk_1: "Hợp đồng có hiệu lực từ 01/01/2024"
    chunk_3: "Bên A phải thanh toán trong vòng 30 ngày kể từ ngày hóa đơn"
        ↓
3️⃣  Gộp context:
    Context = """
    Hạn thanh toán: ngày 31/12/2024, được gia hạn tối đa 30 ngày
    Phương thức thanh toán: chuyển khoản ngân hàng hoặc tiền mặt
    Hợp đồng có hiệu lực từ 01/01/2024
    Bên A phải thanh toán trong vòng 30 ngày kể từ ngày hóa đơn
    """
        ↓
4️⃣  Gửi tới Ollama:
    Prompt = """
    Bạn là trợ lý phân tích hợp đồng.
    Trả lời CHỈ dựa trên context sau:

    Context:
    [chunk text ở trên]

    Câu hỏi: Hạn thanh toán của hợp đồng là ngày bao nhiêu?
    """
        ↓
🤖 Ollama generate:
    "Hạn thanh toán của hợp đồng là ngày 31/12/2024,
     với khả năng gia hạn tối đa 30 ngày."
        ↓
📤 Stream từng chữ về frontend (SSE - Server-Sent Events)
```

#### 🏗️ Vì sao cần tất cả các "layer" này?

| Layer | Nhiệm vụ | Tại sao cần |
|-------|----------|-------------|
| Flask | Nhận HTTP request | Giao tiếp với browser, phải respond nhanh |
| Redis | Message queue | Tách biệt task nặng, tránh Flask bị treo |
| Celery | Background worker | Xử lý nặng (trích PDF, OCR, tính vector) mà không block |
| fastembed | Tính vector nhanh | CPU-based, không cần GPU, rất nhanh |
| ChromaDB | Lưu + tìm vector | Tìm chunks liên quan trong `<10ms` |
| Ollama | Chạy LLM | Xử lý text với context nhỏ → nhanh, ít tài nguyên |
...
✅ KIẾN TRÚC RAG (hiện tại):
   ...
   Chat lần 1 → ChromaDB lấy 4 chunks (10ms) + Ollama process (2s) = 2s
   Chat lần 2 → ChromaDB lấy 4 chunks (10ms) + Ollama process (2s) = 2s
   Tài liệu 1000 trang → Hoạt động bình thường, vì chỉ xử lý 4 chunks
```

#### 🎓 Tóm lại — Luồng hoàn chỉnh

```mermaid
graph LR
    A["👤 User upload PDF"] --> B["🔌 Flask (0.2s)"]
    B --> C["🔴 Redis (Task)"]
    C --> D["👷 Celery Worker"]
    D --> E["📖 Trích text + Cắt chunks"]
    E --> F["📊 fastembed (vector)"]
    F --> G["💾 ChromaDB (lưu index)"]

    H["❓ User: 'Hạn thanh toán?'"] --> I["🔌 Flask RAG"]
    I --> J["📊 fastembed (vector question)"]
    J --> K["🔍 ChromaDB (tìm 4 chunks tương tự)"]
    K --> L["🤖 Ollama (generate answer)"]
    L --> M["📤 Stream response"]
    M --> N["👀 Frontend nhận"]
```

> Mỗi bước này không thể bỏ được vì:
> - **Flask** — phải respond HTTP nhanh.
> - **Redis/Celery** — xử lý nặng ngầm.
> - **fastembed** — tính vector siêu nhanh.
> - **ChromaDB** — tìm nhanh context liên quan.
> - **Ollama** — sinh text với context nhỏ → nhanh.
...
3. **Hệ thống Logging đồng bộ (Unified Logging):**
   Được cấu hình tập trung tại [app_factory.py](app_factory.py) sử dụng thư viện `logging` tiêu chuẩn của Python. Logs có định dạng thống nhất bao gồm timestamp rõ ràng, cấp độ ghi log (INFO/ERROR), tên tệp tin và số dòng phát sinh, giúp dễ dàng debug chẩn đoán lỗi trong cả Flask và Celery.
...
5. **Sinh Embeddings theo lô (Batch Embedding Processing):**
   Trong quá trình trích xuất RAG, hệ thống thay thế việc sinh embeddings tuần tự từng chunk (qua endpoint `/api/embeddings` cũ) bằng việc sinh theo từng nhóm (batch 32 chunks) thông qua endpoint `/api/embed` của Ollama. Điều này giúp tận dụng khả năng xử lý đa luồng/song song của GPU/CPU cục bộ và giảm thiểu số lượng kết nối mạng nội bộ.
